...
public class CalendarEx2 {
	...
	public static void main(String[] args) {
		...
		// sdf.format( Date 객체 )
		// * 지정한 날짜 형식에 밎게 Date 객체의 날짜/시간 정보를 문자열로 반환하는 메소드
		...
		int week = lottoCal.get( Calendar.DAY_OF_WEEK );
		int hour1 = lottoCal.get( Calendar.HOUR );
		...
		lottoCal.set(Calendar.AM_PM, 1);	// 오잔 : 0, 오후 : 1
		lottoCal.set(Calendar.HOUR, 9);
		...
		// 로또 구입이 불가한 조건
		// 1. (구입불가) 오전 00시 ~ 06시
		if ( ampm == 0 && hour1 >=0 && hour1 <= 6) {
			System.err.println("구입 가능한 시간이 아닙니다.");
		}
		// 2. (구입불가) 토요일이면서, 오후 8시 이후
		else if( todayOfWeek.equals("토") && ampm == 1 && hour1 >= 8 ) {
			System.err.println("구입 가능한 시간이 아닙니다.");
		}
		else {
			buyYn = true;
		...
		if (buyYn) {
			// 추첨일 구하는 계산
			lottoCal.add(Calendar.DATE, 7 - week );
			String 추첨일 = sdf.format( lottoCal.getTime() );
			System.out.println("추점일 : " + 추첨일);
		} else {
