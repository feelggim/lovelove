package Day11.Ex04_AutoClose;
	...
	public class TrywihtResoirce {

		public static void main(String[] args) {
			
			//try ( 자동으로 자원해제할 객체 생성) { }
			// : finally 구문에서 close() 하지 않아도, 자동으로 지원해제 해준다
			// * AutoClosable 인터페이스를 구현할 클래스이어야 자동으로 자원해제 가능
			...
			try( FileInputStream fis = new FileInputStream("test.txt")) {	
				...
				while ( (read = fis.read() ) != -1) { 
					...
					// FileNotFoundException : 파일이 존재하지 않을 때 발생하는 예의
					e.printStackTrace();			// 예외 발생의 원일을 찾아 단계별로 예외 메시지를 출력
